...
mock.module('../../src/libs/auth.js', () => ({ authMiddleware: noop, requireRole: () => noop }));
mock.module('../../src/libs/permissions.js', () => ({
    requirePermission: () => noop,
    requireAnyPermission: () => noop,
    PERMISSIONS: {},
}));
mock.module('../../src/libs/requestLogger.js', () => ({ withRequestLogger: () => (app) => app }));
...
describe('routes definitions', () => {
    it('defines product routes', async () => {
        const { productsRoutes } = await import('../../src/modules/products/products.routes.js');
        expectRoute(productsRoutes, '/products');
    });

    it('defines pricing routes', async () => {
        const { pricingRoutes } = await import('../../src/modules/pricing/pricing.routes.js');
        expectRoute(pricingRoutes, '/pricing');
    });

    it('defines sales routes', async () => {
        const { salesRoutes } = await import('../../src/modules/sales/sales.routes.js');
    ...
    it('defines outlet routes', async () => {
        const { outletsRoutes } = await import('../../src/modules/outlets/outlets.routes.js');
        expectRoute(outletsRoutes, '/outlets');
    });

    it('defines warehouse routes', async () => {
        const { warehousesRoutes } = await import('../../src/modules/warehouses/warehouses.routes.js');
        expectRoute(warehousesRoutes, '/warehouses');
    });

    it('defines inventory routes', async () => {
        const { inventoryRoutes } = await import('../../src/modules/inventory/inventory.routes.js');
        expectRoute(inventoryRoutes, '/inventory');
    });

    it('defines supplier routes', async () => {
        const { suppliersRoutes } = await import('../../src/modules/suppliers/suppliers.routes.js');
        expectRoute(suppliersRoutes, '/suppliers');
    });

    it('defines purchase order routes', async () => {
        const { purchaseOrdersRoutes } = await import('../../src/modules/purchaseOrders/purchaseOrders.routes.js');
        expectRoute(purchaseOrdersRoutes, '/purchase-orders');
    });

    it('defines reports routes', async () => {
        const { reportsRoutes } = await import('../../src/modules/reports/reports.routes.js');
        expectRoute(reportsRoutes, '/reports');
    });

    it('defines users routes', async () => {
        const { usersRoutes } = await import('../../src/modules/users/users.routes.js');
        expectRoute(usersRoutes, '/users');
    });

    it('defines audit logs routes', async () => {
        const { auditLogsRoutes } = await import('../../src/modules/auditLogs/auditLogs.routes.js');
