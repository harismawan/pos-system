...
import { parseDuration, buildConfig, validateConfig } from '../../src/config/index.js';
...
describe('config/index', () => {
    ...
    it('uses default redact paths when env not provided', () => {
        const config = buildConfig({ DATABASE_URL: 'postgres://user:pass@localhost:5432/testdb' });
    ...
    it('treats empty required values as missing', () => {
        expect(() => validateConfig({ databaseUrl: '' })).toThrow('Missing required environment variables');
    });
    ...
    it('throws when required env is missing', () => {
        ...
        expect(() => validateConfig(buildConfig(env))).toThrow('Missing required environment variables');
    });
});
