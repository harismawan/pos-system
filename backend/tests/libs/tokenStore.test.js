...
describe('libs/tokenStore', () => {
    ...
    it('stores access tokens with hashed keys and TTL', async () => {
        ...
        expect(redisMock.setex.calls[0]).toEqual([`token:access:${userId}:${tokenHash}`, 60, '1']);
    });

    it('stores refresh tokens with hashed keys and TTL', async () => {
        await tokenStore.storeRefreshToken(userId, token, 120);
        expect(redisMock.setex.calls[0]).toEqual([`token:refresh:${userId}:${tokenHash}`, 120, '1']);
        expect(loggerMock.debug.calls.length).toBe(1);
    });

    it('logs and rethrows when storing tokens fails', async () => {
        redisMock.setex.mockImplementation(async () => { throw new Error('redis down'); });
        await expect(tokenStore.storeAccessToken(userId, token, 1)).rejects.toThrow('redis down');
        expect(loggerMock.error.calls.length).toBe(1);
    });

    it('logs and rethrows when storing refresh token fails', async () => {
        redisMock.setex.mockImplementation(async () => { throw new Error('refresh fail'); });
        await expect(tokenStore.storeRefreshToken(userId, token, 1)).rejects.toThrow('refresh fail');
        expect(loggerMock.error.calls.length).toBe(1);
    });
    ...
    it('validates access tokens using hashed keys', async () => {
        ...
        expect(redisMock.exists.calls[0]).toEqual([`token:access:${userId}:${tokenHash}`]);
    });

    it('returns false when access token validation errors', async () => {
        redisMock.exists.mockImplementation(async () => { throw new Error('boom'); });
        const isValid = await tokenStore.validateAccessToken(userId, token);
    ...
    it('validates refresh tokens using hashed keys', async () => {
        ...
        expect(isValid).toBe(true);
        expect(redisMock.exists.calls[0]).toEqual([`token:refresh:${userId}:${tokenHash}`]);
    });

    it('returns false when refresh token validation errors', async () => {
        redisMock.exists.mockImplementation(async () => { throw new Error('fail'); });
        const isValid = await tokenStore.validateRefreshToken(userId, token);
        expect(isValid).toBe(false);
        expect(loggerMock.debug.calls.length).toBe(1);
    });
    ...
    it('revokes access and refresh tokens', async () => {
        ...
        expect(redisMock.del.calls[0]).toEqual([`token:access:${userId}:${tokenHash}`]);

        await tokenStore.revokeRefreshToken(userId, token);
        expect(redisMock.del.calls[1]).toEqual([`token:refresh:${userId}:${tokenHash}`]);
    });

    it('rethrows on revoke errors and logs error', async () => {
        redisMock.del.mockImplementation(async () => { throw new Error('del fail'); });
        await expect(tokenStore.revokeAccessToken(userId, token)).rejects.toThrow('del fail');
        expect(loggerMock.error.calls.length).toBe(1);
    });

    it('logs and rethrows when revoking refresh token fails', async () => {
        redisMock.del.mockImplementation(async () => { throw new Error('refresh del'); });
        await expect(tokenStore.revokeRefreshToken(userId, token)).rejects.toThrow('refresh del');
        expect(loggerMock.error.calls.length).toBe(1);
    });
    ...
    it('rethrows and logs when revokeAllUserTokens fails', async () => {
        redisMock.keys.mockImplementation(async () => { throw new Error('keys failed'); });
        await expect(tokenStore.revokeAllUserTokens(userId)).rejects.toThrow('keys failed');
        expect(loggerMock.error.calls.length).toBe(1);
    });
});
