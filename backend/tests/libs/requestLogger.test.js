...
const { withRequestLogger, requestLogger } = await import('../../src/libs/requestLogger.js');
...
describe('libs/requestLogger', () => {
    ...
    it('assigns request id and logs request/response with headers/body when enabled', async () => {
        ...
        await callbacks.after[0]({ request, body: { hello: 'world' }, response, set, store });
        await flush();
    ...
    it('uses warn/error levels based on status codes', async () => {
        ...
        await callbacks.after[0]({ request, body: null, response: 'fail', set: { status: 404 }, store });
        await flush();
        expect(loggerMock.warn.calls.length).toBe(1);

        await callbacks.after[0]({ request, body: null, response: 'boom', set: { status: 500 }, store });
        await flush();
    ...
    it('formats multipart bodies when logging request body', async () => {
        ...
        await callbacks.before[0]({ request, store });
        await callbacks.after[0]({ request, body: 'filedata', response: null, set: {}, store });
        await flush();
        const log = loggerMock.info.calls[0][0];
    ...
    it('logs limited headers when full headers disabled', async () => {
        ...
        await callbacks.before[0]({ request, store });
        const set = {};
        await callbacks.after[0]({ request, body: null, response: null, set, store });
        await flush();
        const log = loggerMock.info.calls[0][0];
    ...
    it('omits requestBody when body is missing even if logging enabled', async () => {
        ...
        await callbacks.before[0]({ request, store });
        await callbacks.after[0]({ request, body: undefined, response: 'ok', set: {}, store });
        await flush();
        const log = loggerMock.info.calls[0][0];
    ...
    it('logs response body as string when not an object', async () => {
        ...
        await callbacks.before[0]({ request, store });
        await callbacks.after[0]({ request, body: null, response: 'plain-text', set: {}, store });
        await flush();
        const log = loggerMock.info.calls[0][0];
