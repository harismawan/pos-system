...
describe('libs/auth token helpers', () => {
    ...
    it('generates and verifies access tokens', () => {
        const token = authModule.generateAccessToken({ userId: 'user-1', role: 'OWNER' });
    ...
    it('throws on invalid access token', () => {
        ...
        expect(() => authModule.verifyAccessToken(bad)).toThrow('Invalid or expired token');
    });

    it('throws specific error when refresh token expired', () => {
        const expired = jwt.sign({ userId: 'u1', exp: Math.floor(Date.now() / 1000) - 10 }, process.env.REFRESH_TOKEN_SECRET);
        const err = (() => {
            try {
    ...
    it('throws on invalid refresh token', () => {
        ...
        expect(() => authModule.verifyRefreshToken(invalid)).toThrow('Invalid or expired refresh token');
    });
});
...
describe('libs/auth middleware', () => {
    ...
    it('rejects missing authorization header', async () => {
        const set = {};
        const result = await authModule.authMiddleware({ headers: {}, set, store: {} });
    ...
    it('rejects expired access token', async () => {
        ...
        const set = {};
        const result = await authModule.authMiddleware({ headers: { authorization: `Bearer ${expired}` }, set, store: {} });
        expect(set.status).toBe(401);
    ...
    it('rejects invalid access token format', async () => {
        const set = {};
        const result = await authModule.authMiddleware({ headers: { authorization: 'Bearer bad' }, set, store: {} });
        expect(set.status).toBe(401);
    ...
    it('rejects revoked tokens', async () => {
        ...
        prismaMock.user.findUnique.mockImplementation(async () => ({ outletUsers: [], isActive: true }));
    ...
    it('rejects inactive user', async () => {
        ...
        prismaMock.user.findUnique.mockResolvedValue({ isActive: false, outletUsers: [] });
        const set = {};
        const result = await authModule.authMiddleware({ headers: { authorization: `Bearer ${token}` }, set, store: {} });
        expect(set.status).toBe(401);
        expect(result.error).toBe('User not found or inactive');
    });

    it('rejects outlet without access for non-admin roles', async () => {
        const token = jwt.sign({ userId: 'user-1', role: 'CASHIER' }, process.env.JWT_SECRET);
        tokenStoreMock.validateAccessToken.mockResolvedValue(true);
        prismaMock.user.findUnique.mockResolvedValue({
            ...
            role: 'CASHIER',
            isActive: true,
            outletUsers: [{ outlet: { id: 'out-1', name: 'Outlet', code: 'O1' }, outletId: 'out-1', outletRole: 'CASHIER', isDefaultForUser: false }],
        });
        const set = {};
        const store = {};
        const result = await authModule.authMiddleware({ headers: { authorization: `Bearer ${token}`, 'x-outlet-id': 'out-2' }, set, store });
