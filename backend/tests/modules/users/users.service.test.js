...
const usersService = await import('../../../src/modules/users/users.service.js?controllers');
...
describe('modules/users/users.service', () => {
    ...
    it('rejects creating a user when username already exists', async () => {
        ...
        prismaMock.user.findUnique.mockImplementation(async () => ({ id: 'existing' }));
        prismaMock.user.create.mockImplementation(async () => { createCalled = true; return { id: 'new' }; });

        const err = await usersService.createUser({
            name: 'Jane',
            username: 'jane',
            password: 'secret',
            role: 'ADMIN',
    ...
    it('hashes password and creates user when data is valid', async () => {
        ...
        prismaMock.user.create.mockImplementation(async ({ data }) => ({ ...data, id: 'new-user' }));
        ...
        expect(prismaMock.user.create.calls[0][0].data.passwordHash).toBe('hashed-password');
    });

    it('throws when updating a missing user', async () => {
        prismaMock.user.findUnique.mockImplementation(async () => null);

        await expect(usersService.updateUser('missing', { name: 'Update' }))
    ...
    it('lists users with filters and pagination', async () => {
        ...
        const res = await usersService.getUsers({ page: 2, limit: 5, search: 'a', role: 'ADMIN', isActive: 'true' });
    ...
    it('updates user, hashes password and logs', async () => {
        ...
        prismaMock.user.findUnique.mockImplementation(async ({ where }) => {
            if (where.id) {
                lookup += 1;
                return { id: 'u1', email: lookup === 1 ? 'old@example.com' : 'old@example.com' };
            }
            if (where.email) {
                return null;
            }
        ...
        prismaMock.user.update.mockResolvedValue({ id: 'u1', name: 'Updated', isActive: false });
        bcryptMock.hash.mockResolvedValue('new-hash');

        const user = await usersService.updateUser('u1', { email: 'new@example.com', password: 'new', isActive: false });

        expect(user.id).toBe('u1');
        expect(prismaMock.user.update.calls[0][0].data.passwordHash).toBe('new-hash');
    ...
    it('throws when update email already exists', async () => {
        ...
        await expect(usersService.updateUser('u1', { email: 'dup@example.com' })).rejects.toThrow('Email already exists');
    });

    it('prevents deleting self and missing user', async () => {
        await expect(usersService.deleteUser('u1', 'u1')).rejects.toThrow('Cannot delete your own account');

        prismaMock.user.findUnique.mockResolvedValue(null);
        await expect(usersService.deleteUser('u2', 'admin')).rejects.toThrow('User not found');
    });
    ...
    it('assigns user to outlet and unsets defaults when needed', async () => {
        ...
        prismaMock.outletUser.upsert.mockResolvedValue({ id: 'ou1', outletRole: 'MANAGER' });

        const outletUser = await usersService.assignUserToOutlet('u1', 'out1', 'MANAGER', true);
    ...
    it('throws when assigning outlet for missing user or outlet', async () => {
        ...
        await expect(usersService.assignUserToOutlet('missing', 'out1', 'MANAGER')).rejects.toThrow('User not found');

        prismaMock.user.findUnique.mockResolvedValue({ id: 'u1' });
        await expect(usersService.assignUserToOutlet('u1', 'out-missing', 'MANAGER')).rejects.toThrow('Outlet not found');
    });

    it('removes user from outlet and errors when not assigned', async () => {
        prismaMock.outletUser.deleteMany.mockResolvedValue({ count: 0 });
        await expect(usersService.removeUserFromOutlet('u1', 'out1')).rejects.toThrow('User is not assigned to this outlet');
