...
const outletsService = await import('../../../src/modules/outlets/outlets.service.js');
...
describe('modules/outlets/outlets.service', () => {
    ...
    it('lists outlets with filters and pagination', async () => {
        ...
        const res = await outletsService.getOutlets({ isActive: true, page: 2, limit: 5 });
    ...
    it('returns outlet by id with relations', async () => {
        prismaMock.outlet.findUnique.mockResolvedValue({ id: 'o1', outletUsers: [] });
    ...
    it('throws when outlet not found', async () => {
        ...
        await expect(outletsService.getOutletById('missing')).rejects.toThrow('Outlet not found');
    });

    it('prevents deleting outlets with existing orders', async () => {
        prismaMock.posOrder.count.mockImplementation(async () => 5);

        await expect(outletsService.deleteOutlet('out-1'))
    ...
    it('unsets other defaults when assigning default outlet user', async () => {
        prismaMock.outletUser.upsert.mockImplementation(async (args) => ({ id: 'ou-1', ...args.data }));
    ...
    it('creates an outlet and enqueues audit log', async () => {
        ...
        const result = await outletsService.createOutlet({ name: 'Outlet', outletId: 'o1' }, 'admin');
        expect(result.id).toBe('o1');
        expect(jobsMock.enqueueAuditLogJob.calls.length).toBeGreaterThan(0);
    });

    it('updates an outlet and enqueues audit log', async () => {
        prismaMock.outlet.update.mockResolvedValue({ id: 'o1', outletId: 'o1', name: 'Updated' });
        const result = await outletsService.updateOutlet('o1', { name: 'Updated' }, 'admin');
