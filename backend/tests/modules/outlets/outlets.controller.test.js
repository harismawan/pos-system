...
mock.module('../../../src/modules/outlets/outlets.service.js', () => serviceMock);
mock.module('../../../src/libs/logger.js', () => ({ default: loggerMock }));

const controller = await import('../../../src/modules/outlets/outlets.controller.js');
...
describe('modules/outlets/outlets.controller', () => {
    ...
    it('returns error when list fails', async () => {
        const set = {};
        serviceMock.getOutlets.mockImplementation(async () => { throw new Error('fail'); });
    ...
    it('returns outlet by id', async () => {
        const set = {};
        const res = await controller.getOutletByIdController({ params: { id: 'o1' }, set });
    ...
    it('returns 404 when outlet is missing', async () => {
        const set = {};
        serviceMock.getOutletById.mockImplementation(async () => { throw new Error('Outlet not found'); });

        const res = await controller.getOutletByIdController({ params: { id: 'missing' }, set });
    ...
    it('uses provided status when get outlet fails', async () => {
        ...
        err.statusCode = 410;
        serviceMock.getOutletById.mockImplementation(async () => { throw err; });
    ...
    it('sets 201 when creating outlet', async () => {
        ...
        const res = await controller.createOutletController({ body: { name: 'Outlet' }, store, set });
    ...
    it('returns error when create fails with custom status', async () => {
        ...
        err.statusCode = 422;
        serviceMock.createOutlet.mockImplementation(async () => { throw err; });

        const res = await controller.createOutletController({ body: {}, store, set });
    ...
    it('updates outlet', async () => {
        const set = {};
        const store = { user: { id: 'u1' } };
        const res = await controller.updateOutletController({ params: { id: 'o1' }, body: { name: 'New' }, store, set });
    ...
    it('returns error when update fails', async () => {
        const set = {};
        const store = { user: { id: 'u1' } };
        serviceMock.updateOutlet.mockImplementation(async () => { throw new Error('fail'); });

        const res = await controller.updateOutletController({ params: { id: 'o1' }, body: {}, store, set });
        expect(set.status).toBe(500);
        expect(res.error).toBe('Internal Server Error');
    });
    ...
    it('returns error when delete fails', async () => {
        const set = {};
        serviceMock.deleteOutlet.mockImplementation(async () => { throw new Error('fail'); });

        const res = await controller.deleteOutletController({ params: { id: 'o1' }, set });
        expect(set.status).toBe(500);
        expect(res.success).toBe(false);
    });
    ...
    it('returns outlet users list', async () => {
        const set = {};
        const res = await controller.getOutletUsersController({ params: { id: 'o1' }, set });
        expect(res.success).toBe(true);
    ...
    it('returns error when get outlet users fails', async () => {
        const set = {};
        serviceMock.getOutletUsers.mockImplementation(async () => { throw new Error('fail'); });
