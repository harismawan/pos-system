...
describe('modules/reports/reports.service', () => {
    ...
    it('computes sales summary and groups by month', async () => {
        ...
        prismaMock.posOrder.findMany.mockResolvedValue([
            { id: '1', totalAmount: 100, totalDiscountAmount: 10, totalTaxAmount: 5, createdAt, items: [{ quantity: 2 }] },
            { id: '2', totalAmount: 50, totalDiscountAmount: 0, totalTaxAmount: 2, createdAt, items: [{ quantity: 1 }] },
        ]);
    ...
    it('groups sales by week', async () => {
        ...
        prismaMock.posOrder.findMany.mockResolvedValue([
            { id: '1', totalAmount: 100, totalDiscountAmount: 0, totalTaxAmount: 0, createdAt, items: [{ quantity: 1 }] }
        ]);
    ...
    it('aggregates top products by quantity', async () => {
        prismaMock.posOrderItem.findMany.mockImplementation(async () => [
            { productId: 'p1', quantity: 2, lineTotal: 20, product: { id: 'p1', name: 'A', sku: 'A', category: null }, posOrder: { status: 'COMPLETED' } },
            { productId: 'p1', quantity: 1, lineTotal: 10, product: { id: 'p1', name: 'A', sku: 'A', category: null }, posOrder: { status: 'COMPLETED' } },
            { productId: 'p2', quantity: 5, lineTotal: 50, product: { id: 'p2', name: 'B', sku: 'B', category: null }, posOrder: { status: 'COMPLETED' } },
        ]);
    ...
    it('applies filters and sorts top products by revenue with limit', async () => {
        prismaMock.posOrderItem.findMany.mockResolvedValue([
            { productId: 'p1', quantity: 1, lineTotal: 10, product: { id: 'p1', name: 'A', sku: 'A', category: null } },
            { productId: 'p2', quantity: 1, lineTotal: 20, product: { id: 'p2', name: 'B', sku: 'B', category: null } },
        ]);
    ...
    it('calculates inventory valuation and low stock', async () => {
        prismaMock.inventory.findMany.mockResolvedValue([
            {
                product: { id: 'p1', name: 'A', sku: 'A', category: 'Food', costPrice: 2, basePrice: 5 },
                warehouse: { id: 'w1', name: 'Main', code: 'M1' },
                quantityOnHand: 3,
                minimumStock: 2,
            },
            {
                product: { id: 'p2', name: 'B', sku: 'B', category: 'Other', costPrice: 1, basePrice: 2 },
                warehouse: { id: 'w1', name: 'Main', code: 'M1' },
                quantityOnHand: 1,
                minimumStock: 2,
            },
        ...
        const res = await reportsService.getInventoryValuation({ warehouseId: 'w1', category: 'Food' });
    ...
    it('fetches stock movements with filters and summary', async () => {
        ...
        expect(res.summary[0]).toEqual({ type: 'PURCHASE', count: 1, totalQuantity: 2 });
