...
const serviceMock = {
    ...
    updatePurchaseOrder: createMockFn(async () => ({ id: 'po1', status: 'UPDATED' })),
    receivePurchaseOrder: createMockFn(async () => ({ id: 'po1', status: 'RECEIVED' })),
    cancelPurchaseOrder: createMockFn(async () => ({ id: 'po1', status: 'CANCELLED' })),
};

const loggerMock = { error: createMockFn() };

mock.module('../../../src/modules/purchaseOrders/purchaseOrders.service.js', () => serviceMock);
mock.module('../../../src/libs/logger.js', () => ({ default: loggerMock }));

const controller = await import('../../../src/modules/purchaseOrders/purchaseOrders.controller.js');

describe('modules/purchaseOrders/purchaseOrders.controller', () => {
    beforeEach(() => {
        serviceMock.getPurchaseOrders.mockReset();
        serviceMock.getPurchaseOrders.mockResolvedValue({ orders: [], pagination: {} });
        ...
        serviceMock.updatePurchaseOrder.mockResolvedValue({ id: 'po1', status: 'UPDATED' });
        serviceMock.receivePurchaseOrder.mockReset();
        serviceMock.receivePurchaseOrder.mockResolvedValue({ id: 'po1', status: 'RECEIVED' });
        serviceMock.cancelPurchaseOrder.mockReset();
        serviceMock.cancelPurchaseOrder.mockResolvedValue({ id: 'po1', status: 'CANCELLED' });
    ...
    it('returns error when get purchase orders fails', async () => {
        const set = {};
        serviceMock.getPurchaseOrders.mockImplementation(async () => { throw new Error('fail'); });

        const res = await controller.getPurchaseOrdersController({ query: {}, store: {}, set });
    ...
    it('returns purchase order by id', async () => {
        const set = {};
        const res = await controller.getPurchaseOrderByIdController({ params: { id: 'po1' }, set });
    ...
    it('returns 404 when purchase order missing', async () => {
        const set = {};
        serviceMock.getPurchaseOrderById.mockImplementation(async () => { throw new Error('Purchase order not found'); });

        const res = await controller.getPurchaseOrderByIdController({ params: { id: 'missing' }, set });
    ...
    it('uses provided status when get by id fails', async () => {
        ...
        err.statusCode = 410;
        serviceMock.getPurchaseOrderById.mockImplementation(async () => { throw err; });
    ...
    it('sets 201 on create purchase order', async () => {
        ...
        const res = await controller.createPurchaseOrderController({ body: {}, store, set });
    ...
    it('returns error when create fails with custom status', async () => {
        ...
        err.statusCode = 422;
        serviceMock.createPurchaseOrder.mockImplementation(async () => { throw err; });
    ...
    it('updates purchase order', async () => {
        ...
        expect(res.success).toBe(true);
        expect(serviceMock.updatePurchaseOrder.calls[0]).toEqual(['po1', { status: 'UPDATED' }, 'u1']);
    });

    it('returns error when update fails', async () => {
        const set = {};
        const store = { user: { id: 'u1' } };
        serviceMock.updatePurchaseOrder.mockImplementation(async () => { throw new Error('fail'); });

        const res = await controller.updatePurchaseOrderController({ params: { id: 'po1' }, body: {}, store, set });
        expect(set.status).toBe(500);
        expect(res.error).toBe('Internal Server Error');
    });
    ...
    it('receives purchase order', async () => {
        ...
        expect(res.success).toBe(true);
        expect(serviceMock.receivePurchaseOrder.calls[0]).toEqual(['po1', [], 'u1']);
    });
    ...
    it('returns error when receive fails with status code', async () => {
        ...
        err.statusCode = 409;
        serviceMock.receivePurchaseOrder.mockImplementation(async () => { throw err; });
    ...
    it('cancels purchase order', async () => {
        const set = {};
        const store = { user: { id: 'u1' } };
        const res = await controller.cancelPurchaseOrderController({ params: { id: 'po1' }, store, set });
    ...
    it('returns error when cancel fails', async () => {
        const set = {};
        const store = { user: { id: 'u1' } };
        serviceMock.cancelPurchaseOrder.mockImplementation(async () => { throw new Error('fail'); });
