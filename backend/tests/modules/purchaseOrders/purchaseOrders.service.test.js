...
const purchaseOrdersService = await import('../../../src/modules/purchaseOrders/purchaseOrders.service.js?coverage');
...
describe('modules/purchaseOrders/purchaseOrders.service', () => {
    ...
    it('lists purchase orders with pagination', async () => {
        ...
        const result = await purchaseOrdersService.getPurchaseOrders({ page: 1, limit: 10 });
    ...
    it('throws when purchase order is not found', async () => {
        ...
        await expect(purchaseOrdersService.getPurchaseOrderById('missing'))
            .rejects.toThrow('Purchase order not found');
    });

    it('returns purchase order with related data', async () => {
        prismaMock.purchaseOrder.findUnique.mockResolvedValue({ id: 'po1', supplier: {} });
    ...
    it('creates purchase order and enqueues audit log', async () => {
        ...
        prismaMock.purchaseOrder.create.mockImplementation(async (args) => {
            createArgs = args;
            return { id: 'po1', orderNumber: args.data.orderNumber, items: [], supplier: {}, warehouse: {} };
        });
    ...
    it('updates draft purchase order', async () => {
        prismaMock.purchaseOrder.findUnique.mockResolvedValue({ id: 'po1', status: 'DRAFT' });
        prismaMock.purchaseOrder.update.mockResolvedValue({ id: 'po1', status: 'DRAFT', notes: 'updated' });

        const res = await purchaseOrdersService.updatePurchaseOrder('po1', { expectedDate: '2024-02-02', notes: 'updated' }, 'user-1');
    ...
    it('throws when updating non-draft or missing order', async () => {
        prismaMock.purchaseOrder.findUnique.mockResolvedValue(null);
        await expect(purchaseOrdersService.updatePurchaseOrder('missing', {}, 'u1')).rejects.toThrow('Purchase order not found');

        prismaMock.purchaseOrder.findUnique.mockResolvedValue({ id: 'po1', status: 'RECEIVED' });
        await expect(purchaseOrdersService.updatePurchaseOrder('po1', {}, 'u1')).rejects.toThrow('Can only update draft purchase orders');
    });
    ...
    it('receives purchase order and updates inventory', async () => {
        ...
        prismaMock.purchaseOrder.update.mockResolvedValue({ id: 'po1', status: 'RECEIVED', items: [], supplier: {}, warehouse: {} });

        const res = await purchaseOrdersService.receivePurchaseOrder('po1', [{ itemId: 'item1', quantity: 2 }], 'user-1');
        ...
        expect(jobsMock.enqueueAuditLogJob.calls.at(-1)[0].eventType).toBe('PURCHASE_ORDER_RECEIVED');
    });

    it('throws when receiving already received, cancelled, or missing item', async () => {
        prismaMock.purchaseOrder.findUnique.mockResolvedValue({ id: 'po1', status: 'RECEIVED', items: [] });
        await expect(purchaseOrdersService.receivePurchaseOrder('po1', [], 'u1')).rejects.toThrow('Purchase order already received');

        prismaMock.purchaseOrder.findUnique.mockResolvedValue({ id: 'po1', status: 'CANCELLED', items: [] });
        await expect(purchaseOrdersService.receivePurchaseOrder('po1', [], 'u1')).rejects.toThrow('Cannot receive cancelled purchase order');
        ...
        await expect(purchaseOrdersService.receivePurchaseOrder('po1', [{ itemId: 'missing', quantity: 1 }], 'u1'))
            .rejects.toThrow('Item missing not found in purchase order');
    });

    it('cancels purchase order and enqueues audit log', async () => {
        prismaMock.purchaseOrder.findUnique.mockResolvedValue({ id: 'po1', status: 'DRAFT', outletId: 'out-1', orderNumber: 'PO-1', items: [] });
        prismaMock.purchaseOrder.update.mockResolvedValue({ id: 'po1', status: 'CANCELLED', items: [], supplier: {}, warehouse: {} });

        const res = await purchaseOrdersService.cancelPurchaseOrder('po1', 'user-1');

        expect(res.status).toBe('CANCELLED');
        expect(jobsMock.enqueueAuditLogJob.calls.at(-1)[0].eventType).toBe('PURCHASE_ORDER_CANCELLED');
    });

    it('throws when cancelling received or already cancelled order', async () => {
        prismaMock.purchaseOrder.findUnique.mockResolvedValue({ id: 'po1', status: 'RECEIVED' });
        await expect(purchaseOrdersService.cancelPurchaseOrder('po1', 'u1')).rejects.toThrow('Cannot cancel received purchase order');

        prismaMock.purchaseOrder.findUnique.mockResolvedValue({ id: 'po1', status: 'CANCELLED' });
        await expect(purchaseOrdersService.cancelPurchaseOrder('po1', 'u1')).rejects.toThrow('Purchase order already cancelled');
    });
});
