...
describe('modules/inventory/inventory.service', () => {
    ...
    it('rejects invalid adjustment types', async () => {
        await expect(inventoryService.adjustInventory({
            productId: 'p1',
            warehouseId: 'w1',
            outletId: 'o1',
            quantity: 5,
            type: 'INVALID',
        }, 'user-1')).rejects.toThrow('Invalid adjustment type');
    });
    ...
    it('prevents negative inventory on adjustment out', async () => {
        ...
        await expect(inventoryService.adjustInventory({
            ...
            quantity: 5,
            type: 'ADJUSTMENT_OUT',
        }, 'user-1')).rejects.toThrow('Adjustment would result in negative inventory');
    ...
    it('throws when transferring with same warehouse or insufficient stock', async () => {
        await expect(inventoryService.transferInventory({
            productId: 'p1',
            fromWarehouseId: 'w1',
            toWarehouseId: 'w1',
            outletId: 'o1',
            quantity: 1,
        }, 'user')).rejects.toThrow('Cannot transfer to the same warehouse');
        ...
        await expect(inventoryService.transferInventory({
            ...
            outletId: 'o1',
            quantity: 5,
        }, 'user')).rejects.toThrow('Insufficient inventory for transfer');
    });
    ...
    it('fetches low stock inventory using raw queries', async () => {
        ...
        const result = await inventoryService.getInventory({ lowStock: true, page: 1, limit: 1 });
    ...
    it('throws when source inventory missing for transfer', async () => {
        ...
        await expect(inventoryService.transferInventory({
            ...
            outletId: 'o1',
            quantity: 1,
        }, 'user')).rejects.toThrow('Source inventory not found');
    });
    ...
    it('transfers inventory successfully and enqueues audit log', async () => {
        ...
        prismaMock.inventory.findUnique.mockImplementation(async () => {
            findCount += 1;
            if (findCount === 1) return { productId: 'p1', warehouseId: 'w1', quantityOnHand: 10 };
            return null;
        });
    ...
    it('fetches stock movements with filters', async () => {
        prismaMock.stockMovement.findMany.mockResolvedValue([{ id: 'm1', productId: 'p1' }]);
        prismaMock.stockMovement.count.mockResolvedValue(1);

        const result = await inventoryService.getStockMovements({ warehouseId: 'w1', type: 'SALE', page: 1, limit: 10 });
