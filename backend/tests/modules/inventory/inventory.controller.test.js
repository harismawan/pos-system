...
mock.module('../../../src/modules/inventory/inventory.service.js', () => serviceMock);
mock.module('../../../src/libs/logger.js', () => ({ default: loggerMock }));

const controller = await import('../../../src/modules/inventory/inventory.controller.js');

describe('modules/inventory/inventory.controller', () => {
    beforeEach(() => {
        serviceMock.getInventory.mockReset();
        serviceMock.getInventory.mockResolvedValue({ inventories: [], pagination: {} });
    ...
    it('passes outletId from store to get inventory', async () => {
        ...
        const res = await controller.getInventoryController({ query: {}, store, set });
    ...
    it('returns error response when get inventory fails', async () => {
        const set = {};
        serviceMock.getInventory.mockImplementation(async () => { throw new Error('fail'); });
    ...
    it('returns error response when adjust fails', async () => {
        ...
        serviceMock.adjustInventory.mockImplementation(async () => { throw new Error('fail'); });

        const res = await controller.adjustInventoryController({ body: { productId: 'p1' }, store, set });
        expect(set.status).toBe(500);
    ...
    it('transfers inventory with outlet from store', async () => {
        ...
        const res = await controller.transferInventoryController({ body: { fromWarehouseId: 'w1', outletId: 'q' }, store, set });
    ...
    it('returns error when transfer fails', async () => {
        ...
        serviceMock.transferInventory.mockImplementation(async () => { throw new Error('fail'); });

        const res = await controller.transferInventoryController({ body: { outletId: 'out' }, store, set });
        expect(set.status).toBe(500);
        expect(res.error).toBe('Internal Server Error');
    });
    ...
    it('gets stock movements with outlet from store', async () => {
        ...
        const res = await controller.getStockMovementsController({ query: { outletId: 'q' }, store, set });
    ...
    it('returns error when get stock movements fails', async () => {
        const set = {};
        serviceMock.getStockMovements.mockImplementation(async () => { throw new Error('fail'); });
