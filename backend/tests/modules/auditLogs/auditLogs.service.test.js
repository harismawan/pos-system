...
const auditLogsService = await import('../../../src/modules/auditLogs/auditLogs.service.js?service');
...
describe('modules/auditLogs/auditLogs.service', () => {
    ...
    it('applies date filters when fetching audit logs', async () => {
        ...
        prismaMock.user.findMany.mockImplementation(async () => [{ id: 'u1', name: 'User', username: 'user' }]);
        ...
        expect(prismaMock.auditLog.findMany.calls[0][0].where.createdAt).toBeDefined();
    ...
    it('returns audit log with user info by id', async () => {
        prismaMock.auditLog.findUnique.mockResolvedValue({ id: 'log1', userId: 'u1' });
    ...
    it('throws when audit log is not found', async () => {
        ...
        await expect(auditLogsService.getAuditLogById('missing'))
            .rejects.toThrow('Audit log not found');
    });

    it('returns event types list', async () => {
        prismaMock.auditLog.findMany.mockResolvedValue([{ eventType: 'A' }, { eventType: 'B' }]);
    ...
    it('returns entity types list', async () => {
        prismaMock.auditLog.findMany.mockResolvedValue([{ entityType: 'User' }, { entityType: 'Order' }]);
