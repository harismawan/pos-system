...
const serviceMock = {
    ...
    completePosOrder: createMockFn(async () => ({ id: 'o1', status: 'COMPLETED' })),
...
const controller = await import('../../../src/modules/sales/sales.controller.js');
...
describe('modules/sales/sales.controller', () => {
    beforeEach(() => {
        ...
        serviceMock.completePosOrder.mockResolvedValue({ id: 'o1', status: 'COMPLETED' });
        serviceMock.cancelPosOrder.mockReset();
        serviceMock.cancelPosOrder.mockResolvedValue({ id: 'o1', status: 'CANCELLED' });
    ...
    it('returns POS orders list and prefers outletId from store', async () => {
        const set = {};
        const res = await controller.getPosOrdersController({ query: { outletId: 'q-1' }, store: { outletId: 'store-1' }, set });
    ...
    it('returns error when list fails', async () => {
        const set = {};
        serviceMock.getPosOrders.mockImplementation(async () => { throw new Error('fail'); });

        const res = await controller.getPosOrdersController({ query: {}, store: {}, set });
    ...
    it('sets 201 when creating POS order', async () => {
        ...
        const res = await controller.createPosOrderController({ body: {}, store, set });
    ...
    it('returns error when create fails with status code', async () => {
        ...
        err.statusCode = 422;
        serviceMock.createPosOrder.mockImplementation(async () => { throw err; });
    ...
    it('returns POS order by id', async () => {
        const set = {};
        const res = await controller.getPosOrderByIdController({ params: { id: 'o1' }, set });
    ...
    it('returns 404 when order is missing', async () => {
        const set = {};
        serviceMock.getPosOrderById.mockImplementation(async () => { throw new Error('Order not found'); });

        const res = await controller.getPosOrderByIdController({ params: { id: 'missing' }, set });
    ...
    it('returns 500 and internal message when get by id fails otherwise', async () => {
        const set = {};
        serviceMock.getPosOrderById.mockImplementation(async () => { throw new Error('db down'); });
    ...
    it('handles completion errors with 400 status', async () => {
        const set = {};
        const store = { user: { id: 'u1' } };
        serviceMock.completePosOrder.mockImplementation(async () => { throw new Error('cannot complete'); });

        const res = await controller.completePosOrderController({ params: { id: 'o1' }, store, set });

        expect(set.status).toBe(400);
        expect(res.success).toBe(false);

        serviceMock.completePosOrder.mockImplementation(async () => ({ id: 'o1', status: 'COMPLETED' }));
    });
    ...
    it('cancels POS order', async () => {
        const set = {};
        const store = { user: { id: 'u1' } };
        const res = await controller.cancelPosOrderController({ params: { id: 'o1' }, store, set });
    ...
    it('returns error when cancel fails', async () => {
        const set = {};
        const store = { user: { id: 'u1' } };
        serviceMock.cancelPosOrder.mockImplementation(async () => { throw new Error('cannot cancel'); });
    ...
    it('adds payment with 201', async () => {
        const set = {};
        const res = await controller.addPaymentController({ params: { id: 'o1' }, body: { amount: 10 }, set });
    ...
    it('returns error when add payment fails', async () => {
        const set = {};
        serviceMock.addPayment.mockImplementation(async () => { throw new Error('fail'); });

        const res = await controller.addPaymentController({ params: { id: 'o1' }, body: {}, set });
        expect(set.status).toBe(400);
