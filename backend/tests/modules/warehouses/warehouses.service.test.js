...
const warehousesService = await import('../../../src/modules/warehouses/warehouses.service.js?controllers');
...
describe('modules/warehouses/warehouses.service', () => {
    ...
    it('lists warehouses with filters and pagination', async () => {
        ...
        expect(args.where).toEqual({ outletId: 'out1', type: 'STORE', isActive: true });
        expect(args.skip).toBe(5);
        expect(res.pagination.totalPages).toBe(1);
    });

    it('returns warehouse by id with inventories', async () => {
        prismaMock.warehouse.findUnique.mockResolvedValue({ id: 'w1', inventories: [] });
    ...
    it('throws when warehouse not found', async () => {
        ...
        await expect(warehousesService.getWarehouseById('missing')).rejects.toThrow('Warehouse not found');
    });

    it('prevents deleting warehouses with existing inventory', async () => {
        prismaMock.inventory.count.mockImplementation(async () => 2);

        await expect(warehousesService.deleteWarehouse('w1'))
    ...
    it('unsets other defaults when creating a default warehouse', async () => {
        ...
        prismaMock.warehouse.create.mockImplementation(async ({ data }) => ({ ...data, id: 'w1' }));
    ...
    it('creates warehouse without changing defaults when not requested', async () => {
        prismaMock.warehouse.create.mockResolvedValue({ id: 'w2', outletId: 'o1', name: 'Secondary' });

        const res = await warehousesService.createWarehouse({ name: 'Secondary', outletId: 'o1', isDefault: false }, 'admin');
    ...
    it('updates warehouse and unsets other defaults when needed', async () => {
        prismaMock.warehouse.findUnique.mockResolvedValue({ id: 'w1', outletId: 'o1' });
        prismaMock.warehouse.updateMany.mockResolvedValue({});
        prismaMock.warehouse.update.mockResolvedValue({ id: 'w1', outletId: 'o1', name: 'Updated' });

        const res = await warehousesService.updateWarehouse('w1', { isDefault: true, name: 'Updated' }, 'admin');
    ...
    it('gets warehouse inventory with low stock filter using raw queries', async () => {
        ...
        prismaMock.inventory.findMany.mockResolvedValue([{ id: 'inv1' }, { id: 'inv2' }]);

        const res = await warehousesService.getWarehouseInventory('w1', { lowStock: true, page: 1, limit: 10 });

        expect(res.pagination.total).toBe(2);
        expect(prismaMock.inventory.findMany.calls[0][0].where.id.in).toEqual(['inv1', 'inv2']);
    });
    ...
    it('gets warehouse inventory without low stock filter', async () => {
        ...
        const res = await warehousesService.getWarehouseInventory('w1', { page: 2, limit: 5 });
