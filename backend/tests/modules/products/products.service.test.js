...
const productsService = await import('../../../src/modules/products/products.service.js?controllers');
...
describe('modules/products/products.service', () => {
    ...
    it('throws when product is not found', async () => {
        ...
        await expect(productsService.getProductById('missing')).rejects.toThrow('Product not found');
    });

    it('returns product with related data', async () => {
        prismaMock.product.findUnique.mockResolvedValue({ id: 'p1', inventories: [] });
    ...
    it('updates product', async () => {
        ...
        const product = await productsService.updateProduct('p1', { name: 'Updated' });
