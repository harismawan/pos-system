...
export async function getSalesSummary(filters = {}) {
    ...
    const totalItems = orders.reduce((sum, o) =>
        sum + o.items.reduce((itemSum, item) => itemSum + parseFloat(item.quantity), 0), 0);
    const totalDiscount = orders.reduce((sum, o) => sum + parseFloat(o.totalDiscountAmount), 0);
    const totalTax = orders.reduce((sum, o) => sum + parseFloat(o.totalTaxAmount), 0);
    ...
    orders.forEach(order => {
        ...
        groupedData[key].items += order.items.reduce((sum, item) => sum + parseFloat(item.quantity), 0);
    });
...
export async function getTopProducts(filters = {}) {
    const { startDate, endDate, outletId, limit = 10, sortBy = 'revenue' } = filters;
...
export async function getInventoryValuation(filters = {}) {
    ...
    if (category) {
        filteredInventory = inventory.filter(i => i.product.category === category);
    }
...
export async function getStockMovementReport(filters = {}) {
    const { startDate, endDate, outletId, warehouseId, type, page = 1, limit = 50 } = filters;
...
export async function getOrderHistory(filters = {}) {
    const { startDate, endDate, outletId, status, customerId, page = 1, limit = 20 } = filters;
