...
export async function getPurchaseOrders(filters = {}) {
    const { supplierId, warehouseId, outletId, status, page = 1, limit = 50 } = filters;
...
export async function createPurchaseOrder(data, userId) {
    const { supplierId, warehouseId, outletId, expectedDate, notes, items } = data;
...
export async function receivePurchaseOrder(id, receivedItems, userId) {
    ...
    logger.info({ orderId: order.id, orderNumber: order.orderNumber }, 'Purchase order received');
...
export async function cancelPurchaseOrder(id, userId) {
    ...
    logger.info({ orderId: order.id, orderNumber: order.orderNumber }, 'Purchase order cancelled');
