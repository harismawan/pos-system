...
export async function adjustInventory(data, userId, businessId) {
  ...
  if (!["ADJUSTMENT_IN", "ADJUSTMENT_OUT"].includes(type)) {
    throw new Error(
      "Invalid adjustment type. Use ADJUSTMENT_IN or ADJUSTMENT_OUT",
    );
  }

  const adjustmentQuantity =
    type === "ADJUSTMENT_IN" ? Math.abs(quantity) : -Math.abs(quantity);
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const newQuantity =
      parseFloat(inventory.quantityOnHand) + parseFloat(adjustmentQuantity);
    if (newQuantity < 0) {
