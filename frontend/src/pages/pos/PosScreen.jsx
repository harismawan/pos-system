...
function PosScreen() {
    ...
    const loadWarehouse = async () => {
        try {
            ...
            if (result.warehouses && result.warehouses.length > 0) {
                const defaultWh = result.warehouses.find(w => w.isDefault) || result.warehouses[0];
    ...
    const handleConfirmPayment = async () => {
        try {
            ...
            showNotification(`Order ${order.orderNumber} completed successfully!`, 'success');
            clearOrder();
            ...
            <div className="page-container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
                <div className="card" style={{ textAlign: 'center', maxWidth: '400px' }}>
                    ...
                    <p style={{ color: 'var(--gray-500)' }}>
                        Please select an outlet from the dropdown in the header to start selling.
                    </p>
                </div>
            </div>
        );
        ...
        <div style={{
            ...
            <div style={{
                ...
                <div style={{ marginBottom: '20px' }}>
                    <h1 className="page-title" style={{ marginBottom: '8px' }}>Point of Sale</h1>
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    <h2 style={{ fontSize: '18px', fontWeight: 600, marginBottom: '4px' }}>Current Order</h2>
                    <p style={{ fontSize: '13px', opacity: 0.7 }}>{orderItems.length} items in cart</p>
                </div>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '16px 24px' }}>
                    {orderItems.length === 0 ? (
                        <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--gray-400)' }}>
                            ...
                            <p>Cart is empty</p>
                            <p style={{ fontSize: '13px', marginTop: '4px' }}>Click on products to add them</p>
                        </div>
                    ) : (
                        <div>
                            {orderItems.map((item) => (
                    ...
                    <div style={{
                        ...
                        <div style={{ marginBottom: '16px' }}>
                            ...
                            <div style={{
                                ...
                                <span>Total</span>
                                <span style={{ color: 'var(--primary-600)' }}>Rp {totals.total.toLocaleString('id-ID')}</span>
                            </div>
                        </div>
            ...
            {/* Checkout Modal */}
            {showCheckoutModal && (
                <div className="modal-overlay" onClick={() => !processing && setShowCheckoutModal(false)}>
                    <div className="modal" style={{ maxWidth: '540px' }} onClick={(e) => e.stopPropagation()}>
                        <div className="modal-header" style={{ background: 'linear-gradient(135deg, #1e293b, #334155)', color: 'white' }}>
                            ...
                            <button
                                ...
                                style={{ background: 'rgba(255,255,255,0.2)', fontSize: '18px', color: 'white', width: '32px', height: '32px', borderRadius: 'var(--radius-full)' }}
                                disabled={processing}
                            >
                                ×
                            </button>
                        ...
                        <div className="modal-body" style={{ maxHeight: '60vh', overflowY: 'auto' }}>
                            {/* Order Summary */}
                            <div style={{ marginBottom: '20px' }}>
                                <h4 style={{ fontSize: '14px', fontWeight: 600, color: 'var(--gray-500)', marginBottom: '12px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                                    Order Items
                                </h4>
                                <div style={{ background: 'var(--gray-50)', borderRadius: 'var(--radius-lg)', padding: '12px' }}>
                                    {orderItems.map((item, index) => (
                                        <div key={item.productId} style={{
                                            display: 'flex',
                                            justifyContent: 'space-between',
                                            padding: '8px 0',
                                            borderBottom: index < orderItems.length - 1 ? '1px solid var(--gray-200)' : 'none',
                                        }}>
                                            <div>
                                                <div style={{ fontWeight: 500 }}>{item.product.name}</div>
                                                <div style={{ fontSize: '12px', color: 'var(--gray-400)' }}>
                                                    {item.quantity} × Rp {parseFloat(item.unitPrice).toLocaleString('id-ID')}
                                                </div>
                                            </div>
                                    ...
                                    {[
                                        ...
                                        <button
                                            ...
                                            style={{
                                                padding: '12px',
                                                borderRadius: 'var(--radius-lg)',
                                                border: paymentMethod === method.value ? '2px solid var(--primary-500)' : '1px solid var(--gray-200)',
                                                background: paymentMethod === method.value ? 'var(--primary-50)' : 'white',
                                                cursor: 'pointer',
                                                display: 'flex',
                                                alignItems: 'center',
                                                justifyContent: 'center',
                            ...
                            <div className="form-group">
                                ...
                                >
                                    {staffList.length > 0 ? (
                                        staffList.map((staff) => (
                                            <option key={staff.user?.id || staff.id} value={staff.user?.id || staff.id}>
                                                {staff.user?.name || staff.name} ({staff.outletRole || staff.role})
                                            </option>
                                        ))
                                    ) : (
                                ...
                                <p className="form-helper">Select the staff member responsible for this sale</p>
                            </div>
