...
function InventoryOverviewPage() {
    ...
    const stats = useMemo(() => {
        ...
        const lowStock = inventory.filter(i => parseFloat(i.quantityOnHand) <= parseFloat(i.minimumStock || 10)).length;
        const outOfStock = inventory.filter(i => parseFloat(i.quantityOnHand) <= 0).length;
        const totalValue = inventory.reduce((sum, i) => sum + (parseFloat(i.quantityOnHand) * parseFloat(i.product?.costPrice || 0)), 0);
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            cell: row => (
                <span style={{
                    fontWeight: 600,
                    color: parseFloat(row.quantityOnHand) <= 0 ? 'var(--error-500)' :
                        parseFloat(row.quantityOnHand) <= parseFloat(row.minimumStock || 10) ? 'var(--warning-500)' : 'var(--gray-800)'
                }}>
        ...
        {
            name: 'Value',
            selector: row => parseFloat(row.quantityOnHand) * parseFloat(row.product?.costPrice || 0),
            sortable: true,
            width: '140px',
            cell: row => (
                <span style={{ color: 'var(--gray-600)' }}>
                    Rp {(parseFloat(row.quantityOnHand) * parseFloat(row.product?.costPrice || 0)).toLocaleString('id-ID')}
                </span>
            ),
        },
        {
        ...
        <div className="page-container">
            <div className="page-header">
                <div>
                    ...
                    <p className="page-subtitle">Monitor stock levels across warehouses</p>
                </div>
                <div className="flex gap-3">
                    <button className="btn-secondary" onClick={() => openAdjustModal(null)}>
                        Adjust Stock
                    </button>
                    <button className="btn-primary" onClick={() => openTransferModal(null)}>
                        Transfer Stock
                    </button>
                </div>
            </div>
            ...
            <div className="stats-grid">
                ...
                <div className="stat-card">
                    ...
                    <div className="stat-value" style={{ color: 'var(--warning-500)' }}>{stats.lowStock}</div>
                </div>
                <div className="stat-card">
                    <div className="stat-label">Out of Stock</div>
                    <div className="stat-value" style={{ color: 'var(--error-500)' }}>{stats.outOfStock}</div>
                </div>
                <div className="stat-card">
                    <div className="stat-label">Total Value</div>
                    <div className="stat-value" style={{ fontSize: '22px' }}>Rp {stats.totalValue.toLocaleString('id-ID')}</div>
                </div>
            </div>
            ...
            <div className="filter-bar">
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                    <input
                        ...
                        onChange={(e) => { setFilterLowStock(e.target.checked); setCurrentPage(1); }}
                ...
                <DataTable
                    ...
                    noDataComponent={
                        <div className="empty-state">
                            ...
                            <p>Stock levels will appear here after you make purchases or adjustments</p>
                        </div>
                    }
                />
            </div>
