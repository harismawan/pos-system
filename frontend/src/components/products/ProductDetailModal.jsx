...
function ProductDetailModal({ isOpen, onClose, product: initialProduct }) {
    ...
    const loadData = async () => {
        try {
            setLoading(true);
            const [inventoryResult, movementsResult, productResult] = await Promise.all([
                inventoryApi.getInventory({ productId: initialProduct.id }),
                inventoryApi.getStockMovements({ productId: initialProduct.id, limit: 10 }),
    ...
    const stockColumns = [
        ...
        },
        {
            ...
            cell: row => (
                <span style={{
                    fontWeight: 600,
                    color: parseFloat(row.quantityOnHand) <= parseFloat(row.minimumStock) ? 'var(--warning-600)' : 'var(--success-600)'
                }}>
        ...
        },
        {
            ...
            cell: row => {
                ...
                return qty <= 0 ? <span style={{ color: 'var(--error-600)' }}>Out of Stock</span> :
                    qty <= min ? <span style={{ color: 'var(--warning-600)' }}>Low Stock</span> :
        ...
        },
        {
            ...
            cell: row => {
                ...
                if (row.type === 'PURCHASE' || row.type === 'ADJUSTMENT_IN') {
                    ...
                    bg = 'var(--error-50)'; color = 'var(--error-700)';
                ...
                return <span style={{ background: bg, color: color, padding: '2px 8px', borderRadius: '4px', fontSize: '11px', fontWeight: 500 }}>{label}</span>
            }
        },
        {
            name: 'Qty',
            ...
            cell: row => {
                ...
                return <span style={{ fontWeight: 600 }}>{parseFloat(row.quantity)}</span>
            }
        },
        {
            name: 'Warehouse',
            cell: row => {
                ...
                if (row.type === 'TRANSFER') return <span style={{ fontSize: '11px' }}>{from} → {to}</span>;
                return <span style={{ fontSize: '11px' }}>{to || from || '-'}</span>;
            }
        }
    ];

    const totalStock = warehouseStock.reduce((sum, item) => sum + parseFloat(item.quantityOnHand), 0);
            ...
            <div className="modal" style={{ maxWidth: '800px' }} onClick={(e) => e.stopPropagation()}>
                ...
                <div className="modal-body">
                    ...
                    <div style={{
                        ...
                        <div>
                            <div style={{ fontSize: '11px', color: 'var(--gray-500)', textTransform: 'uppercase' }}>Barcode</div>
                        ...
                        <div>
                            <div style={{ fontSize: '11px', color: 'var(--gray-500)', textTransform: 'uppercase' }}>Total Stock</div>
                            <div style={{ fontWeight: 700, fontSize: '16px' }}>{totalStock} {product.unit}</div>
                        </div>

                        {/* Divider row if needed or just continue grid */}

                        <div>
                            <div style={{ fontSize: '11px', color: 'var(--gray-500)', textTransform: 'uppercase' }}>Base Price</div>
                            <div style={{ fontWeight: 600, color: 'var(--success-700)' }}>Rp {parseFloat(product.basePrice).toLocaleString('id-ID')}</div>
                        </div>
                        <div>
                            ...
                            <div style={{ fontWeight: 600, color: 'var(--gray-700)' }}>
                                {product.costPrice ? `Rp ${parseFloat(product.costPrice).toLocaleString('id-ID')}` : '-'}
                            </div>
                        </div>
                        <div>
                            <div style={{ fontSize: '11px', color: 'var(--gray-500)', textTransform: 'uppercase' }}>Tax Rate</div>
                            <div style={{ fontWeight: 500 }}>{product.taxRate ? `${product.taxRate}%` : '0%'}</div>
                        </div>
                        <div>
                            <div style={{ fontSize: '11px', color: 'var(--gray-500)', textTransform: 'uppercase' }}>Unit</div>
                            <div style={{ fontWeight: 500 }}>{product.unit}</div>
                        </div>

                        {product.description && (
                            <div style={{ gridColumn: '1 / -1', marginTop: '8px', paddingTop: '8px', borderTop: '1px solid var(--gray-200)' }}>
                                <div style={{ fontSize: '11px', color: 'var(--gray-500)', textTransform: 'uppercase', marginBottom: '4px' }}>Description</div>
                                <div style={{ fontSize: '13px', color: 'var(--gray-700)', lineHeight: '1.4' }}>{product.description}</div>
                            </div>
                        )}
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                        {/* Stock by Warehouse */}
                        <div>
                            <h3 style={{ fontSize: '15px', fontWeight: 600, marginBottom: '12px', color: 'var(--gray-700)' }}>Stock by Warehouse</h3>
                            <div className="data-table-container" style={{ boxShadow: 'none', border: '1px solid var(--gray-200)' }}>
                                <DataTable
                                    ...
                                    dense
                                    noDataComponent={<div className="p-4 text-center text-gray-500">No stock records</div>}
                                />
                            </div>
                        </div>
                        ...
                        {/* Recent Movements */}
                        <div>
                            <h3 style={{ fontSize: '15px', fontWeight: 600, marginBottom: '12px', color: 'var(--gray-700)' }}>Recent Activity</h3>
                            <div className="data-table-container" style={{ boxShadow: 'none', border: '1px solid var(--gray-200)' }}>
                                <DataTable
                                    ...
                                    customStyles={customStyles}
                                    dense
                                    noDataComponent={<div className="p-4 text-center text-gray-500">No recent activity</div>}
                                />
                            </div>
                        </div>
                    </div>
