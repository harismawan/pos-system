    ...
    const columns = useMemo(() => [
        {
            ...
            cell: row => (
                <div>
                    ...
                    <div style={{ fontSize: '12px', color: 'var(--gray-500)' }}>SKU: {row.product?.sku}</div>
                </div>
            )
        },
        {
        ...
        {
            name: 'Value',
            selector: row => parseFloat(row.quantityOnHand || 0) * parseFloat(row.product?.costPrice || 0),
            sortable: true,
            width: '130px',
            cell: row => {
                const val = parseFloat(row.quantityOnHand || 0) * parseFloat(row.product?.costPrice || 0);
            ...
            <div className="modal" style={{ maxWidth: '800px' }} onClick={(e) => e.stopPropagation()}>
                <div className="modal-header">
                    <div>
                        ...
                        <div style={{ fontSize: '13px', color: 'var(--gray-500)', marginTop: '4px' }}>
                            {warehouse.city ? `${warehouse.city}, ` : ''}{warehouse.state || ''} • {warehouse.type}
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="modal-body">
                    ...
                    <div style={{
                        ...
                        <div>
                            <div style={{ fontSize: '11px', color: 'var(--gray-500)', textTransform: 'uppercase' }}>Type</div>
                            <div style={{ fontWeight: 500 }}>
                                <span className={`badge ${warehouse.type === 'CENTRAL' ? 'badge-info' : 'badge-neutral'}`}>
                                    {warehouse.type}
                                </span>
                            </div>
                        </div>
                        <div>
                            ...
                            <div style={{ fontWeight: 500 }}>
                                <span className={`badge ${warehouse.isActive ? 'badge-success' : 'badge-error'}`}>
                                    {warehouse.isActive ? 'Active' : 'Inactive'}
                                </span>
                            </div>
                        </div>
                        <div>
                            <div style={{ fontSize: '11px', color: 'var(--gray-500)', textTransform: 'uppercase' }}>Items Held</div>
                            <div style={{ fontWeight: 600, fontSize: '16px' }}>{stats.totalProducts}</div>
                        </div>
                        <div>
                            <div style={{ fontSize: '11px', color: 'var(--gray-500)', textTransform: 'uppercase' }}>Total Value</div>
                            <div style={{ fontWeight: 600, color: 'var(--success-700)', fontSize: '16px' }}>
                    ...
                    <h3 style={{ fontSize: '16px', fontWeight: 600, marginBottom: '12px', color: 'var(--gray-800)' }}>Inventory Holdings</h3>
